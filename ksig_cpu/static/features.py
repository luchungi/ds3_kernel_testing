...
class LowRankFeatures(Kernel, TransformerMixin, metaclass=ABCMeta):
    ...
    Any classes deriving from this at the very least should implement the following methods:
        _make_feature_components(X : ArrayOnCPU): this (void) method should initialize dependent variables (except n_features_)
        _compute_features(X : ArrayOnCPU): this method should compute and return the finite dimensional embedding for input data X
    ...
    def _check_n_features(self, X : ArrayOnCPU, reset : bool = False) -> None:
        ...
        Args:
            X (ArrayOnCPU): Input data as a NumPy or CuPy array.
    ...
    def _validate_data(self, X : ArrayOnCPU, reset : bool = False) -> ArrayOnCPU:
        ...
        Returns:
            ArrayOnCPU: returns the input data as a NumPy or CuPy array without any changes.
        """
        self._check_n_features(X, reset=reset)
        return X

    @abstractmethod
    def _make_feature_components(self, X : ArrayOnCPU) -> None:
    ...
    def fit(self, X: ArrayOnCPU, y : Optional[ArrayOnCPU] = None) -> LowRankFeatures:
    ...
    @abstractmethod
    def _compute_features(self, X : ArrayOnCPU) -> ArrayOnCPU:
        ...
        Returns:
            ArrayOnCPU: A feature set as a CuPy array.
        """
        pass

    def transform(self, X : ArrayOnCPU, return_on_gpu : bool = False) -> ArrayOnCPU:
        ...
        Returns:
            X_feat: (ArrayOnCPU) A feature set as a NumPy or CuPy array.
        """
    ...
    def _K(self, X : ArrayOnCPU, Y : Optional[ArrayOnCPU] = None) -> ArrayOnCPU:
        ...
        Args:
            X (ArrayOnCPU): First input data CuPy array.
            Y (Optional[ArrayOnCPU], optional): Second input data CuPy array. Defaults to None.

        Returns:
            ArrayOnCPU: The kernel matrix between X and Y or X and X as a CuPy array.
        """
        X_feat = self._compute_features(X)
    ...
    def _Kdiag(self, X : ArrayOnCPU) -> ArrayOnCPU:
        ...
        Args:
            X (ArrayOnCPU): Input data CuPy array.

        Returns:
            ArrayOnCPU: The diagonal entries of the kernel matrix between X and X as a CuPy array.
        """
        X_feat = self._compute_features(X)
    ...
    def __call__(self, X : ArrayOnCPU, Y : Optional[ArrayOnCPU] = None, diag : bool = False, return_on_gpu : bool = False) -> ArrayOnCPU:
        ...
        Args:
            ...
            Y (Optional[ArrayOnCPU], optional): Second input data as a NumPy or CuPy array. Defaults to None.
            diag (bool, optional): Whether to compute only the diagonal entries (only if Y is None). Defaults to False.
            return_on_gpu (bool, optional): Whether to return the output on GPU (as CuPy) or on CPU (as NumPy).

        Returns:
            ArrayOnCPU: A kernel matrix or the diagonal entries of a kernel matrix as a NumPy or CuPy array.
        """
        check_is_fitted(self)
        X = self._validate_data(X)
        if Y is not None:
...
class NystroemFeatures(LowRankFeatures):
    ...
    def _make_feature_components(self, X : ArrayOnCPU) -> None:
        ...
        if isinstance(X, ArrayOnCPU):
            basis_inds = np.asnumpy(basis_inds) # doing this seems to be the most efficient
        basis = np.asarray(X[basis_inds])
    ...
    def _compute_features(self, X : ArrayOnCPU) -> ArrayOnCPU:
        ...
        Returns:
            ArrayOnCPU: Nystroem features as a CuPy array.
        """
...
class RBFFourierFeatures(LowRankFeatures):
    ...
    def _make_feature_components(self, X : ArrayOnCPU) -> None:
        ...
        self.n_components_ = self.n_components
        random_state = utils.check_random_state(self.random_state)
        self.random_weights_ = (np.sqrt(2 / self.lengthscale**2) * random_state.normal(size=(self.n_features_, self.n_components_)))
        self.random_offset_ = random_state.uniform(0, 2 * np.pi, size=self.n_components_)
    ...
    def _compute_features(self, X : ArrayOnCPU) -> ArrayOnCPU:
        ...
        projection *= self.sigma * np.sqrt(2.) / np.sqrt(self.n_components)
