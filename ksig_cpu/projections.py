...
from . import utils
from .utils import ArrayOnCPU, ArrayOnCPU, ArrayOnCPU, RandomStateOrSeed
...
class RandomProjection(BaseEstimator, TransformerMixin, metaclass=ABCMeta):
    ...
    Any classes deriving from this at the very least should implement the following methods:
        _make_projection_components(X : ArrayOnCPU, Z : Optional[ArrayOnCPU]): this (void) method should initialize dependent variables (except n_features_)
        _project_features(X : ArrayOnCPU): this method should compute and return the projection applied to input data X
        _project_outer_prod(X : ArrayOnCPU, Z : ArrayOnCPU): this method should compute and return the projection applied to the array outer(X, Z)
    ...
    def _check_batch_dim(self, X : ArrayOnCPU, Z : Optional[ArrayOnCPU] = None) -> None:
        """Base method to check the batch dimensions (all dimensions except the last) of the arrays X and Z match.

        Args:
            X (ArrayOnCPU): First input data as a NumPy or CuPy array.
            Z (Optional[ArrayOnCPU], optional): Second input data as a NumPy or CuPy array. Defaults to None.
    ...
    def _check_n_features(self, X : ArrayOnCPU, Z : Optional[ArrayOnCPU] = None, reset : bool = False) -> None:
        ...
        Args:
            X (ArrayOnCPU): First input data as a NumPy or CuPy array.
            Z (Optional[ArrayOnCPU], optional): Second input data as a NumPy or CuPy array.
    ...
    def _validate_data(self, X : ArrayOnCPU, Z : Optional[ArrayOnCPU] = None, reset : bool = False) -> Tuple[ArrayOnCPU, Optional[ArrayOnCPU]]:
        ...
        Returns:
            Tuple[ArrayOnCPU, Optional[ArrayOnCPU]]: returns the input data(s) as a NumPy or CuPy array without any changes.
        """
        self._check_batch_dim(X, Z=Z)
        self._check_n_features(X, Z=Z, reset=reset)
        return X, Z

    def _check_n_components(self, X : ArrayOnCPU, Z : Optional[ArrayOnCPU] = None) -> None:
    ...
    @abstractmethod
    def _make_projection_components(self, X : ArrayOnCPU, Z : Optional[ArrayOnCPU] = None) -> None:
    ...
    def fit(self, X : ArrayOnCPU, y : Optional[ArrayOnCPU] = None, Z : Optional[ArrayOnCPU] = None) -> RandomProjection:
        """Validates the input data and initializes all dependent variables.

        Args:
            X (ArrayOnCPU): First input data as a NumPy or CuPy array.
            y (Optional[ArrayOnCPU], optional): Only provided for consistency with the sklearn API. Defaults to None.
            Z (Optional[ArrayOnCPU], optional): Second input data as a NumPy or CuPy array. Defaults to None.
    ...
    @abstractmethod
    def _project_features(self, X : ArrayOnCPU) -> ArrayOnCPU:
        ...
        Args:
            X (ArrayOnCPU): Input data as a CuPy array.

        Returns:
            ArrayOnCPU: Sketch of the input data, X, as a CuPy array.
        """
        pass

    @abstractmethod
    def _project_outer_prod(self, X : ArrayOnCPU, Z : ArrayOnCPU) -> ArrayOnCPU:
        """Abstract method for computing and returning the projection for input data outer(X, Z).

        Args:
            X (ArrayOnCPU): First input data as a CuPy array.
            Z (ArrayOnCPU): Second input data as a CuPy array.
        Returns:
            ArrayOnCPU: Sketch of the input data, outer(X, Z), as a CuPy array.
        """
        pass

    def _project(self, X : ArrayOnCPU, Z : Optional[ArrayOnCPU] = None) -> ArrayOnCPU:
        """Wrapper method for computing and returning the projection for input data X or outer(X, Z).

        Args:
            X (ArrayOnCPU): First input data as a CuPy array.
            Z (Optional[ArrayOnCPU], optional): Second input data as a CuPy array.
        Returns:
            ArrayOnCPU: Sketch of the input data, X or outer(X, Z), as a CuPy array.
        """
        if Z is None:
    ...
    def transform(self, X : ArrayOnCPU, Z : Optional[ArrayOnCPU] = None, return_on_gpu : bool = False) -> ArrayOnCPU:
        ...
        Returns:
            ArrayOnCPU: Sketch of the input data, X or outer(X, Z), as a NumPy or CuPy array.
        """
    ...
    def __call__(self, X : ArrayOnCPU, Z : Optional[ArrayOnCPU] = None, return_on_gpu : bool = False) -> ArrayOnCPU:
    ...
    def fit_transform(self, X : ArrayOnCPU, y : Optional[ArrayOnCPU] = None, Z : Optional[ArrayOnCPU] = None,
                      return_on_gpu : bool = False, **fit_params) -> ArrayOnCPU:
...
class GaussianRandomProjection(RandomProjection):
    ...
    def _make_projection_components(self, X : ArrayOnCPU, Z : Optional[ArrayOnCPU] = None) -> None:
        ...
        self.scaling_ = 1. / np.sqrt(self.n_components_)
...
class SubsamplingRandomProjection(RandomProjection):
    ...
    def _make_projection_components(self, X : ArrayOnCPU, Z : Optional[ArrayOnCPU] = None) -> None:
        ...
        self.scaling_ = np.sqrt(self.n_features_ / self.n_components_)
    ...
    def _project_features(self, X : ArrayOnCPU) -> ArrayOnCPU:
        ...
        Returns:
            ArrayOnCPU: Sketch of the input data, X, as a CuPy array.
        """
        return self.scaling_ * np.take(X, self.sampled_idx_, axis=-1)
...
class VerySparseRandomProjection(RandomProjection):
    ...
    def _make_projection_components(self, X : ArrayOnCPU, Z : Optional[ArrayOnCPU] = None) -> None:
        ...
        self.sampled_idx_ = np.where(np.any(utils.robust_nonzero(components_full), axis=0))[0]
        self.n_sampled_ = self.sampled_idx_.shape[0]
        self.components_ = np.squeeze(np.take(components_full, self.sampled_idx_, axis=1))
        self.scaling_ = np.sqrt(1. / (prob_nonzero * self.n_components_))
    ...
    def _project_features(self, X : ArrayOnCPU) -> ArrayOnCPU:
        ...
        return self.scaling_ * utils.matrix_mult(
            np.take(X, self.sampled_idx_, axis=-1).reshape((-1, self.n_sampled_)), self.components_, transpose_Y=True).reshape(X.shape[:-1] + (-1,))
    ...
    def _project_outer_prod(self, X : ArrayOnCPU, Z : ArrayOnCPU) -> ArrayOnCPU:
        ...
        X_proj, Z_proj = utils.subsample_outer_prod_comps(X, Z, self.sampled_idx_)
        return self.scaling_ * utils.matrix_mult(
            np.reshape(X_proj * Z_proj, (-1, self.n_sampled_)), self.components_, transpose_Y=True).reshape(X_proj.shape[:-1] + (-1,))
# ----------------------------------------------------------------------------------------------------------------------------------------------------------------
...
class CountSketchRandomProjection(RandomProjection):
    ...
    def _project_outer_prod(self, X_count_sketch : ArrayOnCPU, Z : ArrayOnCPU) -> ArrayOnCPU:
...
class TensorizedRandomProjection(RandomProjection):
    ...
    def _make_projection_components(self, X : ArrayOnCPU, Z : Optional[ArrayOnCPU] = None) -> None:
        ...
        self.scaling_ = 1. / np.sqrt(self.n_components_) if Z is None else 1.
