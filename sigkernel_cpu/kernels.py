...
class SignatureBase(Kernel, metaclass=ABCMeta):
    ...
    def _validate_data(self, X : ArrayOnCPU, reset : Optional[bool] = False) -> ArrayOnCPU:
...
class SignatureKernel(SignatureBase):
    ...
    def _compute_kernel(self, X : ArrayOnCPU, Y : Optional[ArrayOnCPU] = None, diag : bool = False) -> ArrayOnCPU:
...
class LowRankSignatureKernel(SignatureBase, LowRankFeatures):
    ...
    def _compute_features(self, X : ArrayOnCPU) -> ArrayOnCPU:
        ...
        if self.normalization == 1:
            ...
            P = np.concatenate([p / P_norms[i][..., None] for i, p in enumerate(P)], axis=-1) / np.sqrt(self.n_levels + 1)
        elif self.normalization == 2:
