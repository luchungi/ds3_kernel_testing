...
def signature_kern(M : ArrayOnCPU, n_levels : int, order : int = -1, difference : bool = True, return_levels : bool = False) -> ArrayOnCPU:
...
def signature_kern_first_order(M : ArrayOnCPU, n_levels : int, difference : bool = True, return_levels : bool = False) -> ArrayOnCPU:
    """
...
def signature_kern_low_rank(U : ArrayOnCPU, n_levels : int, order : int = -1, difference : bool = True, return_levels : bool = False,
                            projections : Optional[RandomProjection] = None) -> Union[List[ArrayOnCPU], ArrayOnCPU]:
...
def signature_kern_first_order_low_rank(U : ArrayOnCPU, n_levels : int, difference : bool = True, return_levels : bool = False,
                                        projections : Optional[RandomProjection] = None) -> Union[List[ArrayOnCPU], ArrayOnCPU]:
    """
    ...
    R_sum = np.sum(R.reshape([n_X, l_X, projections[0].n_components, projections[0].rank]), axis=(1, -1)) if projections is not None \
            and isinstance(projections[0], TensorizedRandomProjection) else np.sum(R, axis=1)
    if return_levels:
        P = [P, R_sum]
    else:
    ...
    for i in range(1, n_levels):
        ...
        R_sum = np.sum(R.reshape([n_X, l_X, projections[i].n_components, projections[i].rank]), axis=(1, -1)) if projections is not None \
                and isinstance(projections[i], TensorizedRandomProjection) else np.sum(R, axis=1)
        if return_levels:
            P.append(R_sum)
        else:
...
def signature_kern_higher_order_low_rank(U : ArrayOnCPU, n_levels : int, order : int = -1, difference : bool = True, return_levels : bool = False,
                                         projections : Optional[RandomProjection] = None) -> Union[List[ArrayOnCPU], ArrayOnCPU]:
    """
    ...
    for i in range(1, n_levels):
        ...
        R = R_next
        R_sum = np.sum(R.reshape([d, n_X, l_X, projections[i].n_components, projections[i].rank]), axis=(0, 2, -1)) if projections is not None \
                and isinstance(projections[i], TensorizedRandomProjection) else np.sum(R, axis=(0, 2))
        if return_levels:
            P.append(R_sum)
        else:
            P = np.concatenate((P, R_sum), axis=-1)
